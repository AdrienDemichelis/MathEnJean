#VERSION AVEC QUE DES SORTIES EST ET SUD
...
class Cell:
    ...
    # Modif CS 04022025
    #     Implementation du statut d'une cellule du labyrinth
    def status(self):
        print(self.walls)
        return self.walls
...
class Maze:
    ...
    def open_exit(self):
        ...
        side = random.choice(['S', 'E'])
        cordx = 0
        cordy = 0

        if side == 'S':  # Sortie au sud
...
class Explorer:
    """Représente explorateur testant les différentes possibilités de parcours"""
    ...
    def __init__(self, maz):
        ...
        aux coordonnées (ix, iy).
        """
        self.list  # liste des positions successives de .
        self.ix, self.iy = ix, iy  # Point de départ du labyrinthe.
        # Génère une grille de cellules (tableau 2D).
        self.maze_map = [[Cell(x, y) for y in range(ny)] for x in range(nx)]
